...
this.clearHighlightForLabel = function(label) {
    ...
    this.mesh.traverse((child) => {
        ...
        showFeaturesButton.addEventListener('click', () => {
            ...
            if (this.showFeaturesActive) {
                ...
                // Hide export selected mesh button when show features is toggled on
                if (exportSelectedMeshButton) {
                    exportSelectedMeshButton.style.display = 'none';
                }
                // Show flaggedPoints meshes and enable pointer events
                this.flaggedPoints.forEach(({ mesh }) => {
                    mesh.visible = true;
                });
            } else {
                ...
                // Restore export selected mesh button visibility based on toggledLabels size
                if (exportSelectedMeshButton) {
                    exportSelectedMeshButton.style.display = 'none';
                }
                this.updateExportSelectedMeshButtonVisibility();

                // Hide all delete crosses for flaggedPoints immediately after toggling off show features
                this.flaggedPoints.forEach(({ mesh }) => {
                    if (mesh.userData && mesh.userData.deleteCrossElement) {
                        mesh.userData.deleteCrossElement.style.display = 'none';
                    }
                    // Hide flaggedPoints meshes to disable pointer events
                    mesh.visible = false;
                });

                // Temporarily ignore pointer events for delete crosses to prevent flicker
                this.ignoreDeleteCrossPointerEvents = true;
                setTimeout(() => {
                    this.ignoreDeleteCrossPointerEvents = false;
                }, 4000);
            }
            this.highlightMarkersByLabel();
            this.highlightFacesByLabels();
    ...
    updateExportSelectedMeshButtonVisibility() {
        ...
        // Hide export button if showFeaturesActive is true
        if (this.showFeaturesActive) {
        ...
        this.flaggedPoints.forEach(({ mesh, label: markerLabel }) => {
            if (this.toggledLabels.has(markerLabel)) {
                ...
                if (!mesh.userData.deleteCrossElement) {
                    ...
                    const self = this;
                    this.renderer.domElement.addEventListener('pointermove', (event) => {
                        if (self.ignoreDeleteCrossPointerEvents) {
                            return;
                        }
                        const rect = this.renderer.domElement.getBoundingClientRect();
                        const mouse = new THREE.Vector2(
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add DBSCAN clustering function
    window.viewer.runDBSCANClustering = function() {
        if (!this.flaggedPoints || this.flaggedPoints.length === 0) {
            alert('Ah oh! No marked feature present');
            return;
        }

        // Automatically toggle on show features for visualization without deletable crosses
        this.showFeaturesActive = true;
        this.toggledLabels.clear();
        this.flaggedPoints.forEach(fp => {
            this.toggledLabels.add(fp.label);
            fp.mesh.visible = true;
        });

        // Suppress showing delete crosses during clustering visualization
        this.ignoreDeleteCrossPointerEvents = true;

        // Extract positions of flaggedPoints
        const points = this.flaggedPoints.map(fp => {
            const pos = fp.mesh.position;
            return [pos.x, pos.y, pos.z];
        });

        // Parameters for DBSCAN
        const epsilon = 1.0; // distance threshold, adjust as needed
        ...
        function dbscan(points, eps, minPts) {
            ...
            const noise = new Set();
            const clusterLabels = new Array(points.length).fill(undefined);

            function regionQuery(pointIdx) {
                const neighbors = [];
                const [x1, y1, z1] = points[pointIdx];
                ...
                while (i < neighbors.length) {
                    ...
                    if (!visited[nIdx]) {
                        ...
                        if (nNeighbors.length >= minPts) {
                            neighbors = neighbors.concat(nNeighbors.filter(n => !neighbors.includes(n)));
                        }
                    }
                    if (clusterLabels[nIdx] === undefined) {
                        clusterLabels[nIdx] = clusterId;
                    }
                    i++;
                }
            }
        ...
        // Update marker colors based on cluster assignment
        this.flaggedPoints.forEach((fp, idx) => {
            const clusterId = clusterLabels[idx];
            if (clusterId === undefined) {
                // Noise points colored gray
                fp.mesh.material.color.set(0x888888);
            } else {
                const color = colors[clusterId % colors.length];
                fp.mesh.material.color.set(color);
            }
        });

        alert(`DBSCAN clustering completed. Found ${Math.max(...clusterLabels) + 1} clusters.`);

        // Re-enable delete crosses pointer events after clustering visualization
        setTimeout(() => {
            this.ignoreDeleteCrossPointerEvents = false;
        }, 4000);
    };

    // Add K-means clustering function
    window.viewer.runKMeansClustering = function() {
        if (!this.flaggedPoints || this.flaggedPoints.length === 0) {
            alert('Ah oh! No marked feature present');
            return;
        }
        ...
        const k = 3; // Number of clusters, can be parameterized
        ...
        while (changed && iterations < maxIterations) {
            ...
            for (let i = 0; i < points.length; i++) {
                ...
                let bestCluster = -1;
                for (let c = 0; c < k; c++) {
                    const dist = distance(points[i], centroids[c]);
                    if (dist < minDist) {
                        minDist = dist;
                        bestCluster = c;
                    }
        ...
        this.flaggedPoints.forEach((fp, idx) => {
            ...
            if (clusterId === -1) {
                // Unassigned points colored gray
                fp.mesh.material.color.set(0x888888);
            } else {
                const color = colors[clusterId % colors.length];
                fp.mesh.material.color.set(color);
        ...
        alert(`K-means clustering completed with ${k} clusters.`);
...
// Add Mean-shift clustering function
window.viewer.runMeanShiftClustering = function() {
    if (!this.flaggedPoints || this.flaggedPoints.length === 0) {
        alert('Ah oh! No marked feature present');
        return;
    }
    ...
    const bandwidth = 1.0; // Bandwidth parameter, adjust as needed
    const maxIterations = 100;
    ...
    let shiftedPoints = points.map(p => [...p]);
    let iterations = 0;
    let maxShift = Infinity;

    while (iterations < maxIterations && maxShift > stopThreshold) {
        maxShift = 0;
        for (let i = 0; i < shiftedPoints.length; i++) {
            let numerator = [0, 0, 0];
            let denominator = 0;
            for (let j = 0; j < points.length; j++) {
                const dist = distance(shiftedPoints[i], points[j]);
                const weight = gaussianKernel(dist, bandwidth);
                numerator[0] += points[j][0] * weight;
                numerator[1] += points[j][1] * weight;
                numerator[2] += points[j][2] * weight;
                denominator += weight;
            }
            const newPoint = [
            ...
            const shiftDist = distance(shiftedPoints[i], newPoint);
    ...
    // Cluster assignment: points with shifted points close enough belong to same cluster
    const clusterThreshold = bandwidth / 2;
    let clusters = [];
    let assignments = new Array(points.length).fill(-1);
    ...
    for (let i = 0; i < shiftedPoints.length; i++) {
        ...
        for (let c = 0; c < clusters.length; c++) {
            if (distance(shiftedPoints[i], clusters[c]) < clusterThreshold) {
                assignments[i] = c;
                assigned = true;
                break;
            }
    ...
    alert(`Mean-shift clustering completed. Found ${clusters.length} clusters.`);
    ...
    // Setup clustering dropdown toggle button
    const clusteringButton = document.getElementById('clustering-method-button');
    const clusteringDropdown = document.getElementById('clustering-method-dropdown');
    if (clusteringButton && clusteringDropdown) {
        clusteringButton.addEventListener('click', () => {
            if (clusteringDropdown.style.display === 'block') {
                clusteringDropdown.style.display = 'none';
            } else {
                clusteringDropdown.style.display = 'block';
            }
        });

        // Hide dropdown if clicking outside
        document.addEventListener('click', (event) => {
            if (!clusteringButton.contains(event.target) && !clusteringDropdown.contains(event.target)) {
                clusteringDropdown.style.display = 'none';
            }
        });

        // Add click listeners for dropdown items
        const items = clusteringDropdown.querySelectorAll('.dropdown-item');
        items.forEach(item => {
            item.addEventListener('click', () => {
                const method = item.getAttribute('data-method');
                clusteringDropdown.style.display = 'none';
                // Remove the message "Clustering ..." after clicking on any option
                // clusteringButton.textContent = `Clustering ▼`;
                // Placeholder for clustering method handlers
                if (method === 'dbscan') {
                    if (window.viewer && typeof window.viewer.runDBSCANClustering === 'function') {
                        window.viewer.runDBSCANClustering();
                    } else {
                        alert('DBSCAN clustering function not implemented yet.');
                    }
                } else if (method === 'kmeans') {
                    if (window.viewer && typeof window.viewer.runKMeansClustering === 'function') {
                        window.viewer.runKMeansClustering();
                    } else {
                        alert('K-means clustering function not implemented yet.');
                    }
                } else if (method === 'meanshift') {
                    if (window.viewer && typeof window.viewer.runMeanShiftClustering === 'function') {
                        window.viewer.runMeanShiftClustering();
                    } else {
                        alert('Mean-shift clustering function not implemented yet.');
                    }
                }
            });
        });
